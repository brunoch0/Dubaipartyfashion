...
- 사진 1장 → AI 스튜디오 컷 워크플로우 시연: 상세페이지 샘플 + 인스타 캠페인 샘플 + 모델 교체 12컷 세트 (docs/samples/)
- 인수인계 4단계 로드맵 합의 방향: ①Claude+어드민 ②SEO 글쓰기 루틴 ③Higgsfield 이미지 ④독립 운영
- **파트너 개인사업자 등록 완료** — 도메인/결제(PG)/정산을 사업자 명의로 진행 가능해짐. 다음: 사업자 정보 확인 → 사이트 푸터 표기, 도메인 구매(사업자/파트너 명의)
- 파트너 도메인 구매 완료 — 이름/구매처 전달 대기, 연결은 Bruno
- **판매 전략 단순화 (중요 결정)**: 첫 드롭은 홈페이지 결제 없이 인스타→와츠앱 DM 직접판매. 결제/PG/주문관리 전부 보류 (와츠앱 월 30건+ 반복 시 재검토). 홈페이지 = SEO 아티클 + 브랜드 스토리 아카이브 — docs/로드맵_첫드롭.md v2
- Events 페이지 숨김: 메뉴에서 제거 + 시드 이벤트 draft 전환 (런칭 파티 때 복구 — Header.tsx 주석 한 줄 + 이벤트 공개)
- **쇼케이스 모드 확정 (투트랙)**: 판매 = 인스타→와츠앱 / 홈페이지 = 구경(About·Journal·Lookbook + 대기명단)으로 데이터 축적. Shop 메뉴·사전예약 섹션도 숨김 (복구: Header.tsx 주석 해제 + 어드민에서 사전예약 상태 변경)
...
- 계정: 화이트리스트 이메일은 /admin/login 회원가입 탭에서 셀프 가입 (후속 보강으로 대체됨)
...
- [x] 관리자(어드민): 완료 (2026-07-23, 감사 로그 포함)
